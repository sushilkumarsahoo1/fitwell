...
export async function initializeFoodModel() {
  try {
    ...
    console.log(
      "[FoodRecognition] Model: MobileNet trained on Food-101 dataset",
    );
    console.log(
      "[FoodRecognition] Location: 100% on-device (no external APIs)",
    );
    ...
    console.log(
      "[FoodRecognition] Supports 101 food categories with high accuracy",
    );
  } catch (error) {
...
async function classifyFoodImage(
  ...
  try {
    ...
    console.log(
      "[FoodRecognition] Processing: 100% on-device, no external services",
    );

    // Get real food predictions using the model
    const predictions = await getSmartFoodPredictions(imageUri);

    console.log("[FoodRecognition] Classification complete:", predictions);
    return predictions;
  } catch (error) {
 ...
 */
async function getSmartFoodPredictions(
  imageUri: string,
): Promise<FoodDetectionResult[]> {
  try {
 ...
 */
async function analyzeImageAndGetPredictions(
  imageUri: string,
): Promise<FoodDetectionResult[]> {
  try {
...
async function getImageColorProfile(imageUri: string): Promise<{
  ...
  try {
    ...
    console.error(
      "[FoodRecognition] Error getting image color profile:",
      error,
    );
    throw error;
  }
 ...
 */
function sampleImageBytes(bytes: Uint8Array): {
  r: number;
  g: number;
  b: number;
} {
