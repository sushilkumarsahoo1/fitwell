...
export const FoodRecognitionScreen: React.FC = () => {
  ...
  const handlePickImage = async (source: "camera" | "library") => {
    try {
      console.log(`[FoodRecognition] Starting image picker - Source: ${source}, Platform: ${Platform.OS}`);
      ...
      if (source === "camera") {
        ...
        if (Platform.OS === "ios" && !Platform.isPad) {
          console.log("[FoodRecognition] iOS simulator detected - trying camera...");
        }
        ...
        try {
          ...
          console.warn("[FoodRecognition] Camera failed (common on simulator):", cameraError);
          Alert.alert(
            "Camera unavailable",
        ...
        try {
          ...
          console.error("[FoodRecognition] Gallery picker failed:", libraryError);
          Alert.alert(
            "Gallery unavailable",
      ...
      if (error instanceof Error) {
        if (error.message.includes("permission")) {
          ...
          errorMessage = "Camera not available on simulator. Use Gallery instead.";
        } else {
  ...
  const handleTestMode = async () => {
    console.log("[FoodRecognition] Test mode activated - using demo predictions");
  ...
  const detectFood = async (imageUri: string) => {
    ...
    try {
      ...
      console.log("[FoodRecognition] Mock detection result:", mockFoodDetections);
      setDetectedFoods(mockFoodDetections);
  ...
  const handleLogFood = async () => {
    ...
    try {
      ...
      await supabase.from("food_logs").insert({
        ...
        calories: Math.round(
          selectedItem.calories_per_serving * servingRatio,
        ),
        protein_g: Math.round(
          (selectedItem.protein_g || 0) * servingRatio * 10,
        ) / 10,
        carbs_g: Math.round(
          (selectedItem.carbs_g || 0) * servingRatio * 10,
        ) / 10,
          ...
          <View style={styles.inputRow}>
            <View style={styles.inputContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setQuantity((q) => String(Math.max(10, parseInt(q) - 50)))}
              >
